package io.github.wolverine14.hexshadowitems.registry
...
typealias RegistrarEntry<T> = ShadowitemsRegistrar<T>.Entry<out T>

abstract class ShadowitemsRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Shadowitems.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is ShadowitemsRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
